...
class AuthorizeHandler(HTTPMethod):
    ...
    def get(self, ar=None):
        ...
        if not grant:
            ...
            chars = [c for c in chars if c.has_sufficient_key]

            # These are used ONLY for determining which API key type and mask should be recommended to the user if they
            # don't meet requirements.
            sample_char = u.primary if u.primary else u.characters[0]
            sample_key = sample_char.credentials[0]
                    
            if chars:
                default = u.primary if u.primary in chars else chars[0]
            else:
                return ('brave.core.template.authorize',
                dict(success=False, message=_("You do not have any API keys on your account which match the requirements for this service. Please add an {1} API key with a mask of <a href='/key/mask/{0}'>{0}</a> or better, please add an API key with that mask to your account.".format(sample_key.recommended_mask, sample_key.recommended_kind)),
                     ar=ar))
